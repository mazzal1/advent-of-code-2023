...
def get_seed_ranges(almanac_seeds: list[int]):
    seed_ranges = []
    for i, number in enumerate(almanac_seeds):
        if is_even(i):
            seed_range_start = number
        if is_odd(i):
            seed_range_length = number
            seed_ranges.append({
                "range_start": seed_range_start,
                "range_length": seed_range_length
            })
    return seed_ranges
